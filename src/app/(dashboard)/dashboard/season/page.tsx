...
function ScoreRing({ score, color }: { score: number; color: string }) {
  ...
  return (
    <svg width="52" height="52" viewBox="0 0 52 52">
      ...
      />
      <text x="26" y="30" textAnchor="middle" fontSize="11" fontWeight="bold" fill="#111827">{score}</text>
    </svg>
  )
}
      ...
      <motion.div variants={stagger} initial="initial" animate="animate" className="space-y-6 max-w-2xl mx-auto">
        <motion.div variants={fadeUp}>
          <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
            <Crown size={24} className="text-amber-600" /> Temporada
          </h1>
          <p className="text-gray-500 text-sm mt-1">Elige un modo y lanza tu temporada de transformación</p>
        </motion.div>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  selected ? 'border-transparent' : 'border-gray-200 bg-gray-50 hover:border-gray-300'
                )}
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <p className="font-black text-gray-900 text-base">{meta.label}</p>
        ...
        {/* Duration + launch */}
        <motion.div variants={fadeUp} className="p-5 rounded-2xl border border-gray-100 bg-gray-50 space-y-4">
          <div>
            <p className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2">
              <Calendar size={14} className="text-gray-500" /> Duración
            </p>
              ...
              {[1, 2, 3, 6, 9, 12].map(m => (
                <button
                  ...
                  className={cn(
                    ...
                    months === m ? 'text-gray-900' : 'border-gray-100 text-gray-500 hover:text-gray-700'
                  )}
                  style={months === m ? {
          ...
          <div className="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-gray-100">
            <div>
              ...
              <p className="text-sm font-bold text-gray-900">{format(new Date(), "d 'de' MMMM yyyy", { locale: es })}</p>
            </div>
    ...
    <motion.div variants={stagger} initial="initial" animate="animate" className="space-y-5 max-w-2xl mx-auto">
      ...
      <motion.div variants={fadeUp} className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-gray-900">{activeSeason.name}</h1>
            <p className="text-xs text-gray-500">
              {progress ? `Día ${progress.daysPassed} de ${progress.daysTotal}` : ''}
              {streak > 0 && <span className="text-amber-600 font-semibold ml-2">🔥 {streak} días seguidos</span>}
            </p>
          </div>
        </div>
        <button onClick={() => setConfirmEnd(true)} className="text-xs text-gray-400 hover:text-gray-500 transition-colors flex items-center gap-1">
        ...
        <motion.div variants={fadeUp}>
          <div className="p-4 rounded-2xl border border-gray-100 bg-gray-50">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-gray-500">Progreso de la temporada</span>
            ...
            <div className="flex justify-between mt-1">
              <span className="text-[10px] text-gray-400">{format(new Date(activeSeason.startDate), "d MMM", { locale: es })}</span>
              <span className="text-[10px] text-gray-400">{format(new Date(activeSeason.endDate), "d MMM yyyy", { locale: es })}</span>
            </div>
          </div>
        </motion.div>
      )}
      ...
      {/* Today checklist */}
      <motion.div variants={fadeUp}>
        <div className="p-5 rounded-2xl border bg-gray-50" style={{ borderColor: `${meta.color}30` }}>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-bold text-gray-900">Hoy — {format(new Date(), "EEEE d", { locale: es })}</h3>
            <div className="flex items-center gap-3">
            ...
            {checkItems.map(item => (
              <div key={item.key} className="flex items-center gap-3">
                <div className={cn(
                  ...
                  item.done ? 'bg-emerald-100' : 'bg-gray-50'
                )}>
                  {item.done
                ...
                <span className={cn('text-sm flex-1', item.done ? 'text-gray-900' : 'text-gray-500')}>
                  {item.label}
                </span>
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isToday ? 'border-gray-200' : 'border-gray-100',
                  )}
                ...
                >
                  ...
                  {!isFuture && log ? (
                    ...
                    <div className="w-6 h-6 rounded-lg bg-gray-50 flex items-center justify-center">
                      <span className="text-gray-400 text-[9px]">{isFuture ? '' : '—'}</span>
                    </div>
                  )}
                </div>
              )
      ...
      <motion.div variants={fadeUp} className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={s.label} className="p-4 rounded-2xl border border-gray-100 bg-gray-50 text-center">
          ...
          <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl bg-gray-50 border border-gray-100">
            <div
              ...
              className={cn(
                ...
                logSpiritual ? 'bg-violet-100 border-violet-500' : 'border-gray-200'
              )}
            >
              {logSpiritual && <Check size={11} className="text-violet-600" />}
            </div>
            <span className="text-sm text-gray-700">🙏 Práctica espiritual completada</span>
          </label>
      ...
      <Modal open={confirmEnd} onClose={() => setConfirmEnd(false)} title="¿Terminar temporada?">
        <div className="space-y-4">
          <p className="text-sm text-gray-500">Esta acción marcará la temporada como finalizada. El historial se conserva.</p>
